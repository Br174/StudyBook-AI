...
import { detectChaptersFromPages, readSourceFile } from './lib/documentParserV08.js';
import { buildStudyBook, refineParagraphWithAi, summaryLevels } from './lib/studyEngineV08.js';
...
function importStatus(update) {
  ...
  if (update.phase === 'rich-assets') return `${update.format || 'Documento'} · recupero immagini e tabelle…`;
...
function buildStructureStats(chapters, pages, assets = []) {
  return {
...
function assetLabel(asset) {
  if (asset?.type === 'table') return 'Tabella';
  if (asset?.type === 'image') return 'Immagine';
  return 'Elemento';
}

export default function AppV08() {
  ...
  async function processFile(file) {
    ...
    try {
      ...
      const visualSuffix = assets ? ` · ${assets} elementi visivi preservati` : '';
      if (unresolved) setStatus(`Documento analizzato · OCR recuperato su ${recovered} pagine · ${unresolved} da verificare${visualSuffix}`);
      else if (recovered) setStatus(`Documento analizzato · OCR completato su ${recovered} pagine${visualSuffix}`);
      else setStatus(`Documento analizzato · struttura pronta${visualSuffix}`);
    } catch (err) {
    ...
    <main className="app-shell">
      <header className="hero">
        <div className="hero-copy">
          <span className="eyebrow">STUDYBOOK AI · v0.8</span>
          <h1>Il tuo libro, reso più semplice da studiare.</h1>
          <p>Importa PDF, DOCX, EPUB, testo o fotografie. StudyBook AI conserva struttura e materiale visivo utile, crea sintesi DSA e salva tutto nella Libreria.</p>
        </div>
      ...
      <section className="panel import-panel">
        <div>
          ...
          <p>PDF, DOCX, EPUB, TXT e immagini. Tutti i formati confluiscono nello stesso motore di studio.</p>
        </div>
        ...
        {fileName && (
          <div className="file-name">
            {fileName}
            {documentData?.sourceFormat && <span className="format-chip">{documentData.sourceFormat}</span>}
          </div>
        )}
      ...
      {documentData && (
        <>
          <section className="stats-grid">
            ...
            <article className="stat-card"><span>Visuali</span><strong>{documentData.assets?.length || 0}</strong></article>
          </section>

          {documentData.assets?.length > 0 && (
            <section className="panel asset-panel">
              <div className="asset-panel-head">
                <div>
                  <span className="section-kicker">MATERIALE VISIVO</span>
                  <h2>Immagini e tabelle preservate</h2>
                  <p>Gli elementi utili del documento restano associati al libro e vengono salvati nella Libreria insieme a testo, capitoli e sintesi.</p>
                </div>
                <span className="asset-count-pill">{documentData.assets.length}</span>
              </div>
              <div className="asset-grid">
                {documentData.assets.slice(0, 12).map((asset, index) => (
                  <article className="asset-card" key={`${asset.sourcePath || asset.name || asset.type}-${index}`}>
                    {asset.type === 'image' && asset.dataUrl ? (
                      <div className="asset-image-wrap"><img src={asset.dataUrl} alt={asset.alt || asset.name || `Immagine ${index + 1}`} /></div>
                    ) : asset.type === 'table' ? (
                      <div className="asset-table-preview">
                        <table><tbody>{asset.rows?.slice(0, 4).map((row, rowIndex) => <tr key={rowIndex}>{row.slice(0, 4).map((cell, cellIndex) => <td key={cellIndex}>{cell}</td>)}</tr>)}</tbody></table>
                      </div>
                    ) : null}
                    <div className="asset-card-body">
                      <strong>{asset.name || `${assetLabel(asset)} ${index + 1}`}</strong>
                      <small>{assetLabel(asset)}{asset.sourcePart ? ` · ${asset.sourcePart}` : ''}</small>
                    </div>
                  </article>
                ))}
              </div>
              {documentData.assets.length > 12 && <div className="asset-note">Altri {documentData.assets.length - 12} elementi sono conservati nel progetto del libro.</div>}
              {documentData.assetsTruncated > 0 && <div className="asset-note">Per evitare file troppo pesanti, {documentData.assetsTruncated} elementi molto grandi o eccedenti il limite non sono stati incorporati.</div>}
            </section>
          )}
