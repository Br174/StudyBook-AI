...
export default function AppV14() {
  ...
  function scheduleScannerPageSave(page, delay = 0) {
    ...
    if (delay > 0) timers.set(page.id, setTimeout(run, delay));
    else run();
  }
  ...
  async function openScanner() {
    ...
    try {
      ...
      setCameraOpen(true);
      setStatus(`Scanner pronto · prossima pagina ${scanPagesRef.current.length + 1}`);
    } catch {
  ...
  function patchScanPage(id, patch, { persistDelay = 0 } = {}) {
    const nextPages = setScanPagesNow((pages) => pages.map((page) => (
  ...
  async function retryScanPage(id) {
    if (scanProcessing || generating) return;
    const page = scanPagesRef.current.find((item) => item.id === id);
    const index = scanPagesRef.current.findIndex((item) => item.id === id);
    if (!page) return;
  ...
  async function finishScanSession() {
    const currentPages = scanPagesRef.current;
    if (!currentPages.length || scanProcessing || generating) return;
    if (currentPages.some((page) => page.status !== 'ready' || !page.text.trim())) {
      setError('Prima di terminare, correggi o elimina le pagine che non sono state riconosciute.');
      return;
    }

    const pages = currentPages.map((page, index) => ({
      pageNumber: index + 1,
      text: page.text.trim(),
      source: 'camera-ocr',
    }));
    ...
    const sourceData = {
      fullText,
      pages,
      chapters: detectChaptersFromPages(pages),
      needsOcr: [],
      ...
      structure: {
        ...
        hierarchyEngine: 'v11-scanner',
        persistentScanner: true,
      },
    };
    ...
    <main className="app-shell">
      <header className="hero">
        <div className="hero-copy">
          <span className="eyebrow">STUDYBOOK AI · v0.14</span>
      ...
      <section className="panel import-panel">
        <div>
          ...
          <p>PDF, DOCX, EPUB, TXT e immagini. Lo Scanner multipagina salva automaticamente foto, ordine e testo OCR: puoi chiudere l’app e riprendere più tardi.</p>
        </div>
      ...
      {scanPages.length > 0 && (
        <section className="panel scan-basket">
          <div className="scan-basket-head">
            <div>
              <span className="eyebrow dark">CARTELLINA SCANSIONI · SALVATAGGIO AUTOMATICO</span>
              <h2>{scanPages.length} {scanPages.length === 1 ? 'pagina' : 'pagine'} · {scanReadyCount} pronte</h2>
              <p>Controlla ordine e testo OCR. Foto e OCR restano sul dispositivo per 30 giorni: se interrompi, la raccolta viene ripristinata alla riapertura.</p>
            </div>
                ...
                <div className="scan-page-body">
                  <div className="scan-page-title">
                    ...
                    <span className={`scan-status ${page.status}`}>
                      {page.status === 'ready' ? (page.text.trim() ? 'Pronta' : 'Testo vuoto') : page.status === 'error' ? 'Da riprendere' : 'OCR…'}
                    </span>
                  </div>
          ...
          {scanHasErrors && <p className="scan-warning">Le pagine interrotte restano salvate: premi “Riprova OCR” oppure eliminale prima di terminare.</p>}
        </section>
      )}
      ...
      {cameraOpen && (
        <div className="camera-overlay" role="dialog" aria-modal="true" aria-label="Scanner pagina">
          <div className="camera-sheet">
            ...
            <p className="camera-help">Tieni il foglio diritto, ben illuminato e dentro il riquadro. Dopo lo scatto la pagina viene letta con OCR e salvata nella cartellina. Se chiudi l’app, potrai riprendere dalla stessa pagina.</p>
